...
from utils.math._math import diff1d_periodic
...
class TestMath(unittest.TestCase):
    ...
    def test_diff1d_periodic(self):
        """ test the diff1d_periodic function """
        for dtype in (np.int, np.uint, np.double):
            x = np.array([0, 1, 2], dtype)
            np.testing.assert_equal(diff1d_periodic(x, 3), np.ones(3, np.int))
            np.testing.assert_equal(diff1d_periodic(x, 3.5), np.r_[1, 1, 1.5])
            np.testing.assert_equal(diff1d_periodic(x, 4), np.r_[1, 1, -2])
            np.testing.assert_equal(diff1d_periodic(x, 4.5), np.r_[1, 1, -2])
            
        self.assertRaises(TypeError, lambda: diff1d_periodic(np.array('s'), 1))
    ...
    def test_range_alternating(self):
        ...
        self.assertSequenceEqual(list(m.range_alternating(0)), [])
