...
import numpy as np
from scipy import integrate, optimize, misc, special, stats
...
def mean_std_frequency_table(frequencies, values=None, ddof=0):
    ...
    None, it is set to `values = np.arange(len(frequencies))`. Alternatively,
    `frequencies` can be of type `collections.Counter`, in which case `values`
    must be None.
    `ddof` is the  delta degrees of freedom, which is used in the formula for
        the standard deviation. 
    """
    if isinstance(frequencies, collections.Counter):
        if values is not None:
            raise ValueError('`values` must be None when `frequencies` are of '
    ...
    else:  # frequencies should be a sequences of numbers    
        if values is None:
            values = np.arange(len(frequencies))
        if len(frequencies) != len(values):
            raise ValueError('`frequencies` and `values` must be of equal '
                             'length')
